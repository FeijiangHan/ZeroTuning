...
# ----------------------------- Our Method: ZeroTuning ----------------------------- #

def ZeroTuning_Attention(attn_weights, input_len, self):
    question_start, question_end, rate, target_layers, target_heads = input_len

    # Early exit if no scaling is needed
    if rate == 1:
        return attn_weights
    ...
    # Apply modification only to target layers
    if target_layers is None or self.layer_idx in target_layers:
        scale_matrix = torch.ones_like(modified_attn_weights)

        # Determine which attention heads to modify
        if target_heads is None:
            heads_to_modify = slice(None)  # Target all heads
        else:
            heads_to_modify = target_heads

        # Apply scaling to the initial token or a specified range
        if not question_start and not question_end:
            scale_matrix[:, heads_to_modify, :, 0] = rate
        else:
            scale_matrix[:, heads_to_modify, :, question_start:question_end] = rate

        # Apply the scaling factor
        modified_attn_weights = modified_attn_weights * scale_matrix

        # Re-normalize the weights to sum to 1
        modified_attn_weights[:, heads_to_modify] = modified_attn_weights[:, heads_to_modify] / modified_attn_weights[:, heads_to_modify].sum(dim=-1, keepdim=True)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        input_len: Optional[Tuple[int, int, float, Optional[List[int]], Optional[List[int]]]] = None,
        **kwargs,
        ...
        if input_len is not None:
            # ----------------------------- Our Method: ZeroAttentionReCalibration  ----------------------------- #
            attn_weights = ZeroTunning(attn_weights, input_len, self)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            # è®¾ç½®å½“å‰å±‚çš„ç´¢å¼•
            decoder_layer.self_attn.layer_idx = idx
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        # ä¿æŒinput_lenå‚æ•°åœ¨ç”Ÿæˆè¿‡ç¨‹ä¸­çš„ä¼ é€’
        ...
        model_inputs.update(
            {
                ...
                "attention_mask": attention_mask,
                "input_len": input_len,  # æ·»åŠ input_lenåˆ°model_inputs
            }
        )
        return model_inputs
